...
function main()
	...
	if nTime - HD_TASK_GROUP:GetTask(HD_TASK_GROUP.Limit) < 600 then
		...
		Say(format("<color=green>%s:<color>", "Small Mount")..format("Cßn %d phót %d gi©y cã thÓ nhÊp vµo r­¬ng tiÕp theo", floor(nLast / 60), mod(nLast, 60)), 0);
		return 0;
	end
	if GetNpcName(GetTargetNpc()) ~= "Small Mount" then
		return 0;
	end
	gf_Modify("Exp", 5000000);
	gf_AddItemEx2({2,1,30721,1}, "B¶o R­¬ng Thó C­ng", "Ho¹t ®éng 09/2014", "Mid-Autumn Lantern Festival", 0, 1);
