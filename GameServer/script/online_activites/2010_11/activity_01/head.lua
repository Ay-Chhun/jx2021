...
NT_TitleTable = 
{
	...
	{"Stomachache", 65, 24, 2},
--	{"Chãng MÆt", 65, 25, 2},
	{"Strong and Robust Body", 65, 26, 6},
	{"Tinh ThÇn Ho¹t B¸t", 65, 27, 6},
};

NT_ItemTable = 
{
	{"Runny nose medicine", 2, 1, 30244, 10000},
	{"Cold medicine", 2, 1, 30245, 10000},
	{"Headache medicine", 2, 1, 30246, 10000},
	{"Thuèc tiªu ch¶y", 2, 1, 30247, 10000},
	{"Stomachache medicine", 2, 1, 30248, 10000},
	{"Cough medicine", 2, 1, 30249, 10000}
};

function NT_Dialog(sayDialog, sayHead)
	if gf_CheckEventDateEx(NT_ActivityID) == 1 then
		tinsert(sayDialog, "Buy medicine/NT_Dialog2");
	end
...
function NT_Dialog2()
	...
	for i=1, getn(NT_ItemTable) do
		tinsert(menu, "Ta muèn mua 1 phÇn"..NT_ItemTable[i][1].."(1 gold)/#NT_DoBuy("..i..")");
	end
	tinsert(menu, "Leave");
...
function NT_AddTitle(type1, type2)
	...
	if 0 ~= type1 and 0 ~= type2 then
		...
		Msg2Player("You feel"..name..", nhËn ®­îc"..name.."hiÖu øng");		
