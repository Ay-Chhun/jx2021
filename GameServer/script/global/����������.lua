...
function Calfroskill(id)
	if (HaveLearnedSkill(id)==1) then	--ÒÑ¾­Ñ§¹ýµ±Ç°¼¼ÄÜ
   		return 1	--Âú×ãÇ°Ìá¼¼ÄÜÌõ¼þ
	elseif  (SKILLS[id]["levelless"][2] == 0)  then	--Ã»ÓÐÇ°Ìá¼¼ÄÜ
		return 1	--Âú×ãÇ°Ìá¼¼ÄÜÌõ¼þ
	...
	if (GetExp() >= needExp)  then	--¼ì²é¾­ÑéÖµ
		if (GetReputation() >= LinearValue(skillLevel + 1, 4, SKILLS[id]["levelless"][1], SKILLS[id])) then	--¼ì²éÉùÍû
			i = Calfroskill(id)	--¼ì²éÇ°Ìá¼¼ÄÜ
		 	if (i == 1) then
		 		if (GetCash() >= (needExp / 2)) then	--¼ì²é½ðÇ®
		 			return 1	--¿ÉÒÔÑ§
		 		else
		 			return 6	--½ðÇ®²»¹»
		 		end
...
function learnskill(id)
	...
	if (HaveLearnedSkill(id) == 0) then
		Say(tipbeforelearn.."Häc vâ c«ng nµy cÇn "..needRep.." ®iÓm danh väng vµ "..needExp.." ®iÓm kinh nghiÖm!"..floor(needExp/2).." ®ång, x¸c ®Þnh chø?",2,"Agree/yeslearn","Skip/no")
	else
		Say(tipbeforelearn.."Häc vâ c«ng nµy cÇn "..needRep.." ®iÓm danh väng vµ "..needExp.." ®iÓm kinh nghiÖm!"..floor(needExp/2).." ®ång, x¸c ®Þnh chø?",2,"Agree/yeslearn","Skip/no")
	end
end
...
function yeslearn()
	...
	if (n==1) then 
		...
		ModifyExp(-needExp)	--¿Û³ý¾­Ñé
 		Pay(floor(needExp/2))	--¿Û³ý½ðÇ®
 
		if (HaveLearnedSkill(id)) == 0 then
 			LearnSkill(id)		--Ñ§»á¼¼ÄÜ
  		else
  			LevelUpSkill(id)	--Éý¼¶¼¼ÄÜ
		end

		afterlearnskill(id)

	end
	showlimit(n)	--·´À¡ÐÅÏ¢¸øÍæ¼Ò
