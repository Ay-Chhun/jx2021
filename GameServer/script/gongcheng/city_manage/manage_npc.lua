...
tTaskInfo =
{
	{30000, "TuÇn nµy thu ®­îc 30000 ®Þch binh Hé gi¸p - Gi¶i th­ëng:", "500 taels"},
	{45000, "TuÇn nµy thu ®­îc 45000 ®Þch binh Hé gi¸p - Gi¶i th­ëng:", "1200 taels"},
	{60000, "TuÇn nµy thu ®­îc 60000 ®Þch binh Hé gi¸p - Gi¶i th­ëng:", "2000 taels"},
	{90000, "TuÇn nµy thu ®­îc 90000 ®Þch binh Hé gi¸p - Gi¶i th­ëng:", "2 medal bags, 1 bag of 50 medals"},
	{150000, "TuÇn nµy thu ®­îc 150000 ®Þch binh Hé gi¸p - Gi¶i th­ëng:", "4 medal bags, 1 bag of 50 medals"},
	{210000, "TuÇn nµy thu ®­îc 210000 ®Þch binh Hé gi¸p - Gi¶i th­ëng:", "6 medal bags, 1 bag of 50 medals"},
...
function task_zhankuang()
	...
	if nType == 0 then
		...
		if szName == szBoss or szName == szManager then
			Say(szSay,3,"L·nh phÇn th­ëng bang héi/task_tong_finish","Hu©n ch­¬ng thµnh thÞ ngÉu nhiªn/task_person_more","End conversation/no_say")
		else
			Say(szSay,2,"Hu©n ch­¬ng thµnh thÞ ngÉu nhiªn/task_person_more","End conversation/no_say")
		end
	end
end
...
function sel_task_weapon_one(nIndex)
	Say("You have chosen"..tTaskWeapon[nIndex][4]..", x¸c ®Þnh chø?",
		3,
...
function task_person_more()
	...
	Say(szMsg.."\nTuÇn nµy b¹n giao ®­îc §Þch Binh Hé Gi¸p <color=yellow>"..nSJCountView.."<color>, trong ®ã sè l­îng cã thÓ ®æi Hu©n ch­¬ng thµnh thÞ lµ <color=yellow>"..nSJCount.."<color>, ®· sö dông <color=yellow>"..nJHCount.."<color>§Þch Binh Hé Gi¸p ®æi Hu©n ch­¬ng thµnh thÞ, phÇn cßn d­ cã thÓ ®æi <color=yellow>"..nCount.."<color> Hu©n ch­¬ng thµnh thÞ! §æi b©y giê chø?",2,"§æi Hu©n ch­¬ng thµnh thÞ/#task_person_more_yes("..nCount..")","End conversation/no_say")
end
...
function task_person_more_yes(nCount)
	if AddItem(2,0,765,nCount) == 1 then
		...
		WriteLog(GetName().."exchange"..nCount.." Hu©n ch­¬ng thµnh thÞ ngÉu nhiªn")
	end
end
...
function task_jiangli_zm(nType,nParam1,nParam2)
	if nType == 0 then
		Say("Ng­¬i cã thÓ chän c¸c phÇn th­ëng sau",
			...
			"End conversation/no_say")
	elseif nType == 1 then
		Say("§æi KhuÊt Nguyªn Béi cÇn 600 Hu©n ch­¬ng thµnh thÞ, ®æi chø?",2,"Agree/#task_jiangli_zm(11,0,0)","Let me think about it!/no_say")
	elseif nType == 2 then
		Say("§æi §¹i Lùc ThÇn H¹ng Liªn cÇn 750 Hu©n ch­¬ng thµnh thÞ, ®æi chø?",2,"Agree/#task_jiangli_zm(12,0,0)","Let me think about it!/no_say")
	elseif nType == 3 then
		...
		if getn(tRoute2Weapon[nRoute]) > 1 then
			...
			Say("Exchange"..tRoute2Weapon[nRoute][1][4].." cÇn 500 Hu©n ch­¬ng thµnh thÞ, ®æi chø?",2,"Agree/#task_jiangli_zm(13,"..nRoute..",1)", "Let me think about it!/no_say")
		end
	elseif nType == 4 then
		Say("§æi L¨ng Ba Vi Bé cÇn 600 Hu©n ch­¬ng thµnh thÞ, ®æi chø?",2,"Agree/#task_jiangli_zm(14,0,0)","Let me think about it!/no_say")
	elseif nType == 10 then
		Say("Exchange"..tRoute2Weapon[nParam1][nParam2][4].." cÇn 900 Hu©n ch­¬ng thµnh thÞ, ®æi chø?",2,"Agree/#task_jiangli_zm(13,"..nParam1..","..nParam2..")", "Let me think about it!/no_say")
...
function task_jiangli_xg_buy(nIndex,nParam)
	if nParam == 0 then
		Say("Exchange"..tMagicAttr[nIndex][4].."Requires"..tMagicAttr[nIndex][5].." Hu©n ch­¬ng thµnh thÞ hoÆc cÇn "..tMagicAttr[nIndex][6].." §Þch Binh Hé Gi¸p, ®æi chø?\n<color=red>NÕu hiÖu qu¶ lo¹i nµy ®· tån t¹i, ®æi lo¹i míi sÏ cã hiÖu qu¶ <color>",
			4,
...
function hui_taishou()
	Say("<color=green>Qu¶n lý thµnh thÞ<color>: C¸c h¹ x¸c nhËn muèn thu håi trang bÞ thµnh chñ trong hµnh trang?", 2, "Agree/yes_hui_taishou", "Let me think about it!/no_say")
end
...
function hui_shiye()
	Say("<color=green>Qu¶n lý thµnh thÞ<color>: C¸c h¹ x¸c nhËn muèn thu håi trang bÞ qu©n s­ trong hµnh trang? ", 2, "Agree/yes_hui_shiye", "Let me think about it!/no_say")
end
...
function hui_tixia()
	Say("<color=green>Qu¶n lý thµnh thÞ<color>: C¸c h¹ x¸c nhËn muèn thu håi trang bÞ danh bé trong hµnh trang?", 2, "Agree/yes_hui_tixia", "Let me think about it!/no_say")
end
...
function hui_weibin()
	Say("C¸c h¹ x¸c ®Þnh muèn thu håi Trang bÞ VÖ binh trong hµnh trang?", 2, "Agree/yes_hui_weibin", "Let me think about it!/no_say")
end
...
function manage_city()
	...
	if nCount == 0 then
		...
		elseif GetTongName() == szTongName then
			Say("Thµnh nµy do Quý bang chiÕm lÜnh", 1, "Leave/no_say")
		else
			local szTongViewName = szTongName
			szTongViewName = sf_Replace(szTongViewName, "/" ,"-")
			szTongViewName = sf_Replace(szTongViewName, "|" ,"-")
...
function yes_get_boss()
	...
	elseif nRet == 0 then
		...
		SaveNow()
		Say("B¹n ®¶m nhËn chøc "..get_tong_sex_name(), 2, "Vµo Qu¶n lý thµnh thÞ/manage_city", "Leave/no_say")
...
function manager_setcess()
	...
	Say("Quý bang ®· thiÕt lËp thuÕ suÊt buèn b¸n lµ <color=yellow>"..(nCessBuysell / 10).."%<color>, and the stall tax rate to <color=yellow>"..(nCessStore / 10).."%<color>. Cã ph¶i muèn thay ®æi?",
...
function jieshao_city()
	Say("ThuyÕt minh qu¶n lý thµnh thÞ", 3, "ThuyÕt minh X­ng hiÖu/chenghao_shuoming", "ThuyÕt minh X­ng hiÖu Trang bÞ/fuli_shuoming", "End conversation/no_say")
end
...
function fuli_shuoming()
	Say("ThuyÕt minh X­ng hiÖu Trang bÞ: Ng­êi ch¬i cã X­ng hiÖu ®Æc biÖt cã thÓ l·nh hoÆc chÕ t¹o trang bÞ ®Æc biÖt, toµn bé thµnh viªn trong bang héi ®Òu cã thÓ l·nh VÖ binh Kh«i gi¸p vµ ngo¹i trang thµnh thÞ", 2, "trë l¹i/jieshao_city", "End conversation/no_say")
end
