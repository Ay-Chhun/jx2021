...
function tmz_single_join()
	...
	--Ê±¼äÏŞÖÆ
--	local nTime = tonumber(date("%H%M"));
--	if (not ((1100 <= nTime and nTime < 1300) or (1500 <= nTime and nTime < 1700))) then
--		Talk(1,"",g_NpcName.."DiÔn vâ ch­a më, tõ 11:00-13:00, 15:00-17:00 h·y quay l¹i b¸o danh tham gia");
--		return 0;
--	end
...
--	local nTime = tonumber(date("%H%M"));
--	if (not ((1100 <= nTime and nTime < 1300) or (1500 <= nTime and nTime < 1700))) then
--		Talk(1,"",g_NpcName.."DiÔn vâ vÉn ch­a chuÈn bŞ xong, thiÕu hiÖp h·y quay l¹i vµo 11: 00 ~ 13: 00 hoÆc 15: 00~17: 00 tham gia chiÕn tr­êng.");
--		return 0;
	if tmz_check_time(1) ~= 1 then
		return 0
	end
...
function tmz_change_weapon_confirm()
	...
	WriteLog("[§æi ®iÓm tİch lòy Thiªn M«n TrËn]: tµi kho¶n: "..tostring(GetAccount()).."Nh©n vËt: "..tostring(GetName()).."Thêi gian: "..tonumber(date("%Y%m%d")).."[Exchange 1 item:"..tAwardWeapon[nRoute][1][1].."]. Kı hiÖu: "..nAdd_flag);
end
...
function tmz_about_detail_4()
	Talk(1,"tmz_about_detail",g_NpcName.."Vµo 12: 00 -14: 00 vµ 18: 00 - 20: 00 mçi ngµy, ng­êi ch¬i ®¨ng kı tham gia ®Êu trËn vµ nhËn ®­îc phÇn th­ëng ®iÓm kinh nghiÖm sÏ ®­îc nh©n ®«i. HiÖu qu¶ nµy kh«ng t¸c dông víi"..tAwardItem[1][1].." and"..tAwardItem[2][1].."Cumulative.")
end
...
function tmz_Special_Service_1(nType,nBattleLv,nTongScore)
	...
	Say(g_NpcName.."X¸c ®Şnh dïng<color>"..tUpLvXoYoYu[nType].."Ngäc Tiªu Dao gióp bang héi n©ng cÊp thµnh "..(nType+1).."No?",
		2,
...
function tmz_tong_award_callback_1(szkey, nkey1, nkey2, nCount,_szkey, _nkey1, _nkey2)
	...
	WriteLog("[ChiÕn tr­êng Thiªn M«n TrËn]: tµi kho¶n: "..tostring(GetAccount())..", name:"..tostring(GetName())..", time:"..tonumber(date("%Y%m%d"))..", [®· nhËn "..nTmzMark.."§iÓm tİch lòy Thiªn M«n TrËn]");
