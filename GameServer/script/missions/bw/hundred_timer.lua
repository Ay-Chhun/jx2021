...
local Bonus_str = "";
	...
	if nTotalFigtherNum == 1 then
		if nDelayState == 0 then
			...
			if nPlayerIndex > 0 then
				...
				WriteLog("[QuÇn hïng chiÕn] Ng­êi ch¬i "..GetName().." wins the battle"..GetMissionV(START_TIME).."nhËn ®­îc phÇn th­ëng:"..nBonus.." Tæng phÝ vµo trËn lµ:"..GetMissionV(MULTIMAP_BONUS).." Account:"..GetAccount());
				PlayerIndex = OldPlayer;
			end
	...
	elseif nSec == 0 then
		Msg2MSAll(MULIT_MISSION_ID, "Giai ®o¹n chiÕn ®Êu: Thi ®Êu ®ang tiÕn hµnh, trong trËn hiÖn cã"..nTotalFigtherNum.." ®Êu thñ, thêi gian thi ®Êu cßn"..nMin.." minutes.");
	end
