...
CSTAGE5.tbBossInfo = {"boss5","Jie Heng Beast"};
...
function CSTAGE5:OnCreateBoss()
	...
	TM_StartTimeGuage("Close the door and release the dogs",60,0,1,tbPlayer);
end;
--Ëæ»úÇøÓòË¢×Ô±¬NPC
function CSTAGE5:CreateOpposedNpc(tbArg)
	...
	local nMapID = SubWorldIdx2ID(SubWorld);
	for i=1,nCount do
		nNpcIdx = CreateNpc("TiÓu qu¸i ¶i 5","Vicious Hounds",nMapID,1569,3175,-1,1,1,200);
		SetNpcDeathScript(nNpcIdx,"\\script\\missions\\tong_mission\\stage\\death_npc.lua");
	end;
	local tbPlayer = TM_GetPlayerTable();
	TM_StopTimeGuage(1);
	TM_StartTimeGuage("Close the door and release the dogs",30,0,1);
end;
...
function CSTAGE5:CreateRandomTrap(nCount)
	...
	for i=1,nCount do
		...
		nRandom = random(1,2)
		nNpcIdx = CreateNpc(tNpcModel[nRandom],"Mechanism",nMapID,self.tbTrap[nRandPosIdx][1],self.tbTrap[nRandPosIdx][2]);
		SetNpcDeathScript(nNpcIdx,"\\script\\missions\\tong_mission\\stage\\death_npc.lua");
	end;
end;
--³õÊ¼»¯tbTimerEven
