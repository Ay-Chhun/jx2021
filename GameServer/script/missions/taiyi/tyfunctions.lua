...
-- function CheckDailyNormalTimes()
	-- local nTimes = GetTask(TASKID_TY_NORMAL_TIMES_LIMIT)
	-- if nTimes > MAX_TIMES_TO_ENTER_NORMAL then
		-- Talk(1, "", format(szNpcName.."C¸c h¹ ®· tham gia ®ñ <color=red>%d lÇn <color> råi.Kh«ng thÓ tiÕp tôc! Ngµy mai h·y quay l¹i!", MAX_TIMES_TO_ENTER_NORMAL));
		-- return 0;
	-- end
	-- return 1;
-- end

-- function CheckDailyHeroTimes()
	-- local nTimes = GetTask(TASKID_TY_HERO_TIMES_LIMIT)
	-- if nTimes > MAX_TIMES_TO_ENTER_HERO then
		-- Talk(1, "", format(szNpcName.."C¸c h¹ ®· tham gia ®ñ <color=red>%d lÇn <color> råi.Kh«ng thÓ tiÕp tôc! Ngµy mai h·y quay l¹i!", MAX_TIMES_TO_ENTER_HERO));
		-- return 0;
	-- end
	-- return 1;
-- end
...
function TY_EnterCondCheck(szNpcName, nMSType, nDifType)
	...
	for i=1,GetTeamSize() do
		PlayerIndex = GetTeamMember(i)
		szName = GetName()
		if GetTask(TASKID_TY_HERO_TIMES_LIMIT) >= MAX_TIMES_TO_ENTER_NORMAL then
			flag = 2;
			break;
		end
	end
	...
	if flag==2 then
		...
		gf_team_message(GetTeamID(), format("Thµnh viªn [%s] trong ®éi ban h«m nay ®· tham gia %d lÇn, mêi chê ngµy mai vÒ l¹i.", szName, MAX_TIMES_TO_ENTER_NORMAL), 3);
		return 0;
	end
...
function TY_CheckMapCountLimit(szNpcName, nTargetMSType, nTargetMSDif)
	...
	for i = 1, getn(tMapId) do
		...
		if not nMSType or not nDifType or not (nMSType == eMSName.FOREST or nMSType == eMSName.TOWER) or not (nDifType == eDifType.NORMAL or nDifType == eDifType.HARD) then
			...
			nDifType = eDifType.NORMAL;
			nMSType = eMSName.FOREST;
		end
