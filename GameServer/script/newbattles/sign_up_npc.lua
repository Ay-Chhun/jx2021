...
function gf_GetLevelRebornCount()
	return gf_NewGetCount()*100+GetLevel()
end
...
function sign_up(nBattleType)	--ÅÐ¶ÏÕ½³¡Ê±¼äÓëÍæ¼ÒµÈ¼¶
	...
	--local nLevelNew = gf_GetLevelRebornCount() or 0
	--if nLevelNew < 692 then
	--	Talk(1,"","<color=green>"..g_sNpcName.."<color>: ChiÕn tr­êng lµ n¬i nguy hiÓm, ®îi ng­¬i luyÖn tËp thªm mét thêi gian ®¹t <color=yellow>chuyÓn sinh 6 cÊp 92<color> råi h·y quay l¹i!");
	--	return 0;
	--end;
	
	--if GetReputation() < 15000 then
	--	Talk(1,"","C¸c h¹ kh«ng ®ñ 15000 ®iÓm danh väng")
	--	return 0;
	--end
	
	--if GetTask(336) < 15000 then
	--	Talk(1,"","C¸c h¹ kh«ng ®ñ 15000 ®iÓm danh väng s­ m«n.")
	--	return 0;
	--end
	...
	local selTab = {
				"Agree/#sign_up_confirm("..nBattleType..",0)",
				"Ta muèn sö dông b¸o danh VIP (kh«ng bÞ giíi h¹n sè ng­êi chªnh lÖch, chØ dµnh cho ng­êi cã Qu©n hµm Nguyªn So¸i, T­íng Qu©n phe "..tCampNameZ[nCamp].."). Note: the registration fee is 99 gold! /#sign_up_confirm("..nBattleType..",1)",
...
--	Msg2Player("B¹n nhËn ®­îc "..tMocRuong[nType][2].." Méc R­¬ng")	
	...
	FireEvent("event_mission_award", "newbattle", nType, is_winner, nLastBattle)
	
	extra_zhanchang_award(nType)--»î¶¯½±Àø
end;
