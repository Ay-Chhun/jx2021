...
function OnDeath(npcIndex)
	...
	if nNpcCamp == SONG_ID then
		Say("Nh©n vËt phe Tèng <color=yellow>"..GetMissionS(MSTR_SONG_TARGETNAME+nNpcCamp-1).."<color> coordinates: <color=yellow>"..floor(MapX/8)..","..floor(MapY/16).."<color>",0);
	else
		Say("Nh©n vËt phe Liªu <color=yellow>"..GetMissionS(MSTR_SONG_TARGETNAME+nNpcCamp-1).."<color> coordinates: <color=yellow>"..floor(MapX/8)..","..floor(MapY/16).."<color>",0);
	end;
end;
