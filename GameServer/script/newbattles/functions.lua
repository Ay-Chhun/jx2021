...
function BT_EmperorAward(nProbability)
	...
	if nRandomNum < nProbability*(10^(nStrLen-nDotPos)) then
		...
		Msg2MSGroup(MISSION_ID,"Player"..GetName().."  lËp ®­îc c«ng lín nªn ®­îc ban th­ëng ®iÓm tİch lòy"..EMPEROR_AWARD_POINT.." ®iÓm, kinh nghiÖm"..EMPEROR_AWARD_EXP.." ®iÓm.",BT_GetCamp());
	end;
end
--Õ½µØÖ®ĞÇ
...
function BT_ProcessBattleOver()
	...
	local tType2Name = {
		[VILLAGE_ID] = "Village Seizure Battle",
		[RESOURCE_ID] = "Grass Valley Seizure Battle",
		[EMPLACEMENT_ID] = "Fortress Seizure Battle",
		[MAINBATTLE_ID] = "ChiÕn tr­êng chİnh-Nh¹n M«n Quan",
	}
...
function confirm_god_of_war()
	...
	WriteLogEx("Tong Lieu","Marshal's summon")		
end
