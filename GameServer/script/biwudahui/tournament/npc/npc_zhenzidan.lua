...
function season_modify_exp(nExp,nCount)
	...
	WriteLog(format("[Grand Martial Tournament"..(nCurSeason-1).."thi quı ] [Sucess] [ModifyExp] [role:%s(acc:%s)] [NhËn ®­îc kinh nghiÖm %d]", GetName(), GetAccount(), nExp*nCount));
end
...
--	end
	...
	local tbRankInfo = {{370/4*g_nBWWeeks,"Tier 1"},{330/4*g_nBWWeeks,"Tier 2"},{270/4*g_nBWWeeks,"Tier 3"},{200/4*g_nBWWeeks,"Tier 4"},{100/4*g_nBWWeeks,"Gi¶i khuyÕn khİch"}};
	local nType = 0;
	local szMsg = "";
...
function season_confirm_award(nType)
	...
	gf_SetLogCaption("");
	local tbRankInfo = {"Tier 1","Tier 2","Tier 3","Tier 4","Gi¶i khuyÕn khİch"};
...
--	end
	if BWT_CheckBiWuTime() ~= 1 then
		Talk(1,"",g_szInfoHead..format("HiÖn nay kh«ng ph¶i thêi gian cuéc dù tuyÓn cña ®¹i héi tû vâ toµn quèc, xin ®¹i hiÖp mçi ngµy trong %s ®Õn ®©y.", "18:00-20:00"));
		return 0;
	end
--	if GetXYB() <= 0 then
	...
	local tBuChangAward = {
		{
			...
			{"Tói §¹i Héi Tû Vâ thi quı 20 c¸i","gf_AddItemEx({2,95,171,20,1}, 'Grand Martial Arts Tournament season bag')"},
