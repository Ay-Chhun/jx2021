...
function marriage_setting()
	...
	-- Ω·ªÈ
	DoMarry(lover)
	WriteLog("[Marriage]:"..GetName().."k’t h´n, bπn ÆÍi lµ"..GetName(lover)..", ÆÈ h∂o h˜u:"..friend_grade..", trﬁ sË t◊nh c∂m ban Æ«u lµ:"..love_grade)
	SaveNow();
	PlayerIndex = lover;
	SaveNow();
...
function add_qingtie_packet(nCount)
	...
	for i=1,nCount do
		...
		WriteLog("[Marriage]:"..GetName().." ß∑ mua 1 thi÷p mÍi ");
	end;
end;
-- «Â≥˝‘§∂©–≈œ¢
function clear_subs()
