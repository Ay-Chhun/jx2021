...
function Init()
	...
	AddMisEvent("event_daily_clean", "Xö lý h»ng ngµy", 0, thisFile, "handle_zk_dailyclean");
	local tMissionName = {
...
ZK_TZQ_NUM_MAX = 3; --ÌôÕ½È¯Ã¿Ìì×î´óÊýÁ¿
ZK_ZMMX_COUNT_MAX = 5; --ÕÆÃÅÃÜÐÅÃ¿ÈÕÍê³É´ÎÊý
...
function handle_zk_main(npcIndex)
	...
	if tGtTask:check_task_isfinish(380) ~= 1 then
		Talk(1,"","H·y hoµn thµnh nhiÖm vô chØ dÉn t¨ng cÊp ChiÕn Cuång råi quay l¹i nhÐ!");
		return 0;
	end
	local szNpcName = GetNpcName(npcIndex);
	local tSay = {
		format("PhiÕu Khiªu ChiÕn Vâ L©m (%d/%d)/#zk_TzqTask(%d)", zk_GetTzqNum(), ZK_TZQ_NUM_MAX, npcIndex),
		format("Tr­êng M«n MËt TÝn-cã thÓ l¾p l¹i(%d/%d)/#zk_ZmmxTask(%d)", zk_GetZmmxCount(), ZK_ZMMX_COUNT_MAX, npcIndex),
	...
	if zkwq_IsOpen() ~= 0 then
		tinsert(tSay, format("Ho¹t ®éng n©ng cÊp vò khÝ ChiÕn Cuång/#zkwq_Main(%d)", npcIndex));
	end
...
function zk_ZmmxTask_do(nIndex)
	...
	Say(format("<color=gold>%s<color>:%s", GetNpcName(GetTargetNpc()), msg), 2,
		format("§ång ý/#zk_ZmmxTask_do_cb(%d)", nIndex), "T¹i h¹ chØ xem qua th«i/nothing");
end
...
function zkwq_Info(npcIndex)
	local tbBody = "\n1. Nguyªn liÖu cÇn:\n\tDung LuyÖn Th¹ch*25, L©n PhiÕn*1, Thiªn M«n Kim LÖnh*3, Thiªn Cang LÖnh*2.\n2. C¸ch nhËn L©n PhiÕn:\n\tNghe nãi gÇn ®©y lóc 9h tèi mçi ngµy, Háa Kú L©n Con vµ ThÇn Thó Háa Kú L©n sÏ chia nhau xuÊt hiÖn ë [Vò L¨ng S¬n 176/180] vµ [Linh B¶o S¬n 224/218]. §¸nh b¹i Háa Kú L©n ®Ó nhËn L©n PhiÕn. Háa Kú L©n Con do ch­a tr­ëng thµnh nªn sÏ r¬i ra Ýt L©n PhiÕn h¬n. L­u ý, Háa Kú L©n lu«n ph¸t ra s¸t khÝ XÝch Viªm ®Ó bao v©y ng­êi ®Õn tiªu diÖt nã, ®Ò nghÞ c¸c vÞ thiÕu hiÖp h·y tæ ®éi ®Ó hç trî nhau. NÕu ®Õn 11h ch­a cã thiÕu hiÖp nµo tiªu diÖt ®­îc nã th× nã sÏ biÕn mÊt.\n3.C¸ch nhËn Dung LuyÖn Th¹ch:\n\tLµ lo¹i vËt liÖu chÕ t¹o ®Æc biÖt, cã thÓ dung luyÖn mäi vËt chÊt. Do L©n PhiÕn ph¶i ®­îc ®un ë nhiÖt ®é cao nªn cÇn dïng ®Õn Dung LuyÖn Th¹ch nµy. Dung LuyÖn Th¹ch rÊt hiÕm, cã thÓ mua ë Ngù C¸c. Nghe nãi ë T­¬ng D­¬ng xuÊt hiÖn mét Th­¬ng Nh©n Chî §en chuyªn mua b¸n c¸c lo¹i nguyªn liÖu quý hiÕm, thiÕu hiÖp cã thÓ ®Õn ®ã xem sao.";
	local tbSay = {};
