...
-- Ê²Ã´Ê±ºò¿ÉÒÔ×ö¸öµ¥´¿µÄ²ß»®£¬²»ÓÃÐ´½Å±¾ÄØ£¬ÓôÃÆing
...
function bosihanfu_killed_1()
	...
	if g_probability(5,100) then
   	AddItem(2,0,686,1,1);--µÃµ½²¨Ë¹Ñý¼§ÒÂ·þÒ»Ì×
...
function xiyuhanfu_killed_1()
	...
	if g_probability(5,100) then
  	AddItem(2,0,687,1,1);--µÃµ½Î÷Óòº·¸¾ÒÂ·þÒ»Ì×
  	if GetItemCount(2,0,686) >= 1 and GetItemCount(2,0,687) >= 1 then
    	TaskTip("§· nhËn ®­îc Yªu C¬ Ba T­ vµ trang phôc phô n÷ T©y Vùc, mçi thø mét bé, mau ®­a cho ¢u D­¬ng Häa. ");
    	Msg2Player("§· nhËn ®­îc Yªu C¬ Ba T­ vµ trang phôc phô n÷ T©y Vùc, mçi thø mét bé, mau ®­a cho ¢u D­¬ng Häa. ");
  	end;
...
function xibeizhuanghan_killed_1()
	...
	if g_probability(15,100) then
  	AddItem(2,0,689,1,1);--µÃµ½ÐÑ¾ÆÒ©
  	if GetItemCount(2,0,689) >= 10 then
  		TaskTip("§· nhËn ®­îc 10 lä thuèc gi¶i r­îu, mau ®­a cho TiÓu NhÞ tÆng T«n NhÞ LiÔn. ");
  		Msg2Player("§· nhËn ®­îc 10 lä thuèc gi¶i r­îu, mau ®­a cho TiÓu NhÞ tÆng T«n NhÞ LiÔn. ");
  	end;
	end;
...
function guanwaigaoshou_killed_1()
	...
	if g_probability(5,100) then
  	...
  	if GetItemCount(2,0,691) >= 1 then
  		TaskTip("LÊy ®­îc 1 th­ mêi cña NhÊt phÈm ®­êng trªn ng­êi cao thñ quan ngo¹i, mau chãng quay vÒ Long M«n trÊn b¸o cho T«n NhÞ LiÔn. ");
  		Msg2Player("LÊy ®­îc 1 th­ mêi cña NhÊt phÈm ®­êng trªn ng­êi cao thñ quan ngo¹i, mau chãng quay vÒ Long M«n trÊn b¸o cho T«n NhÞ LiÔn. ");
  	end;
	end;
...
--É±ËÀÃ÷½Ì¼éÏ¸È¡µÃò¿»ð½ÌÁîÅÆ
function mingjiaojianxi_killed_1()

	if g_probability(5,100) then
  	AddItem(2,0,709,1,1);--µÃµ½ò¿»ð½ÌÁîÅÆ
...
--É±ËÀµÁÄ¹ÈËÈ¡µÃ½ðË¿ÃæÕÖ
function daomuren_killed_1()

	if g_probability(15,100) then
  	AddItem(2,0,710,1,1);--µÃµ½½ðË¿ÃæÕÖ
...
--É±ËÀÎ÷ÏÄÓÂÊ¿»ñµÃ¶ÏÂÞ¸ý
function xixiayongshi_killed_1()

	if g_probability(5,100) then
  	AddItem(2,0,714,1,1);--µÃµ½ÂÞ¸ý
...
function shamoxingzhe_killed_1()
	...
	if g_probability(10,100) then
  	AddItem(2,0,722,1,1);--µÃµ½¡¶Ç§½ðÒí·½¡·²Ð¾í
...
local strTalk = {
	...
	if g_probability(15,100) then
  	...
  	if GetItemCount(2,0,729) >= 10 and GetItemCount(2,0,730) < 1 then
  		...
  		AddItem(2,0,730,1,1);--µÃµ½ÕÙ»½É±ÊÖÍÞÍÞÓÃµÄÃÔ»ê±ùÌÇºùÂ«
