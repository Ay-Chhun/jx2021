...
function main()
	...
	Say("Many thanks!"..Zgc_pub_sex_name().."®Õn b¸i tÕ mé chñ nh©n, cã cÇn tiÓu nh©n ®­a ngµi vÒ kh«ng?",getn(selTab),selTab);
end
--*******************************´«»ØÈı´óÖ÷³Ç*****************************
...
function get_teel_task_prize()
	...
	if task_reel_table[task_diff][1] == 1 then			--É±¹ÖÈÎÎñ
		if GetTask(tomb_sweep_2007_task_reel_step) ~= 1 then
			Talk(1,"","<color=green>Han Clansman<color>:"..Zgc_pub_sex_name().."kh«ng ®­îc nãi bËy tr­íc mé tiªn nh©n, lµm xong nhiÖm vô míi ®Õn ®©y!")
			return
		end
	else
		--ÎïÆ·ÓµÓĞ¼ì²â
		for i = 1,task_reel_table[task_diff][2] do
			...
			if task_reel_table[task_diff][10+add_num] ~= 1 then
				...
				elseif GetItemCount(task_reel_table[task_diff][7+add_num],task_reel_table[task_diff][8+add_num],task_reel_table[task_diff][9+add_num]) > task_reel_table[task_diff][10+add_num] then
					Talk(1,"","You"..task_reel_table[task_diff][6+add_num].."d­ ra <color=yellow>1<color> c¸i, l·o phu kh«ng thÓ nhËn.")
					return
				end
			end
		end
