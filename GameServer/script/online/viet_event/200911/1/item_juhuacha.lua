...
function OnUse(nItemIdx)
	...
	if DelItemByIndex(nItemIdx, 1) == 1 then
		ModifyExp(125000);
		WriteLogEx(VIET_0911_LOG_TITLE,"Chrysanthemum Tea",125000,"®iÓm kinh nghiÖm")
		...
		if mod(GetTask(VIET_0911_TASK_JUHUACHA_COUNT),50) == 0 then
			...
			if nRand == 18 then
				...
				WriteLogEx(VIET_0911_LOG_TITLE,"Chrysanthemum Tea",2,"®iÓm danh väng")
			elseif nRand == 19 then
				SetTask(336, GetTask(336) + 2);
				Msg2Player("B¹n nhËn ®­îc 2 ®iÓm S­ m«n.")
				WriteLogEx(VIET_0911_LOG_TITLE,"Chrysanthemum Tea",2,"®iÓm s­ m«n")
			else
