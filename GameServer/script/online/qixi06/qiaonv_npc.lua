...
function main()
	...
	if nMapID == 100 or nMapID == 200 or nMapID == 300 then
		local selTab = {
				"Ta muèn nhËn Tinh Tinh §Þa §å/get_atlas",
		...
		--Say("<color=green>ÇÉÅ®<color>£ºÊ±ÖµÆßÔÂ³õÆßÈµÇÅ»á£¬¸ÕºÃÒ»¶ÎÒøºÓ»¯ÎªÁ÷ÐÇÓê×¹ÂäÈË¼ä£¬ÕæÊÇÃÀÃî°¡¡£",getn(selTab),selTab);
	else	--ÔÚÒøºÓµØÍ¼
		local selTab = {
...
function get_atlas()
	...
	if nRetCode == 1 then
		...
		WriteLog("[Lçi ho¹t ®éng ThÊt TÞch]:"..GetName().."Lçi khi nhËn Tinh Tinh §Þa §å, trÞ quay l¹i chØ lÖnh AddItem:"..nRetCode);
	end;
end;
...
function know_detail()
	Talk(3,"know_detail_1","Ho¹t ®éng lÇn nµy chia lµm 4 phÇn. <enter><color=yellow>PhÇn 1: Mçi tèi cã thÓ ®Õn Thµnh §«, BiÖn Kinh, TuyÒn Ch©u<color> t×m <color=yellow>X¶o N÷<color> gÇn D· TÈu nhËn <color=yellow>1 Tinh Tinh §Þa §å<color>. Sö dông Tinh Tinh §¹i §å cã thÓ nhËn ®­îc täa ®é, ®Õn täa ®é chØ ®Þnh cña ®Þa ®å chØ ®Þnh sö dông Tinh Tinh §Þa §å cã thÓ nhËn ®­îc ng«i <color=yellow>Sao b¨ng<color>",
...
function know_detail_1()
	Talk(3,"main","<color=yellow><enter>PhÇn 4 §æi phÇn th­ëng: <color><enter>Ng­êi ch¬i ch­a nhËn phÇn th­ëng h«m ®ã vµ th¾p s¸ng c¸c ng«i sao cã thÓ ®èi tho¹i víi <color=yellow>Ng­u Lang hoÆc Chøc N÷<color> nhËn phÇn th­ëng. Ng­êi ch¬i ®· th¾p s¸ng sao cña h«m ®ã cã thÓ ®Õn gÆp <color=yellow>X¶o N÷<color> nhËn 1 phÇn th­ëng nhá.",
					...
					"Chó ý: T×m h¶o h÷u cïng tham gia tæ ®éi sÏ cã ®iÒu bÊt ngê x¶y ra!");
end;
...
function leave_galaxy()
	...
	elseif nMapID == 968 then
		...
		--Msg2Player("ÏµÍ³³ö´í£¬Çë½ØÍ¼ºóµ½ÂÛÌ³ÁªÏµGM£¬Ð»Ð»ÄúµÄºÏ×÷¡£");
		WriteLog("[Lçi ho¹t ®éng ThÊt TÞch]:"..GetName().."Lóc rêi khái map d¶i Ng©n Hµ, IP map nhËn ®­îc sai, trÞ sè lµ:"..nMapID);
		NewWorld(100,1445,2950);	--Ä¬ÈÏ´«»ØÈªÖÝ
	end;
end;
...
function get_award_by_atlas()
	...
	if nAtlasCount <= 0 then
		Talk(1,"","B¹n kh«ng cã Tinh Tinh §Þa §å?");
		return 0;
	end;
	local selTab = {
...
function get_award_confirm(nType,nCount)
	if nType == 1 then
		if DelItem(t7XiItem["atlas"][2],t7XiItem["atlas"][3],t7XiItem["atlas"][4],nCount) == 1 then
			...
			WriteLog("[Ho¹t ®éng ThÊt TÞch]:"..GetName().."Use card"..nCount.."tÊm Tinh Tinh §Þa §å ®æi ®­îc phÇn th­ëng.");
		end;
	else
		if DelItem(t7XiItem["star"][2],t7XiItem["star"][3],t7XiItem["star"][4],nCount) == 1 then
			...
			ModifyReputation(5*nCount,0);
			WriteLog("[Ho¹t ®éng ThÊt TÞch]:"..GetName().."Use card"..nCount.."tÊm Tinh Tinh ®æi ®­îc phÇn th­ëng.");
		end;
	end;
end;
