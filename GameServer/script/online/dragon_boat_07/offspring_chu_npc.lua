...
Include("\\script\\online\\dragon_boat_07\\dragon_boat_08_head.lua")
	...
	Tb_zong_zi_info[1119] = {{"Bµnh İt Hµm Thñy","Nguyªn liÖu hîp thµnh c¬ b¶n v« hiÖu","",70,1000,"delete_all","toµn bé nguyªn liÖu sÏ biÕn mÊt"},{"Wrapping Leaf",1130,1,0},{"Hµm Thñy",1132,1,3},{"Bronze Bucket",1118,1}}
	Tb_zong_zi_info[1120] = {{"İch Trİ Tèng","Lo¹i b¸nh nµy kh«ng cã g× ®Æc biÖt","",70,1500,"delete_all","toµn bé nguyªn liÖu sÏ biÕn mÊt"},{"Wrapping Leaf",1130,1,0},{"İch Trİ Nh©n",1133,1,4},{"Bµnh İt Hµm Thñy",1119,1}}
	Tb_zong_zi_info[1121] = {{"Red Bean Zongzi","Lo¹i b¸nh nµy kh«ng cã g× ®Æc biÖt","",60,2000,"delete_all","toµn bé nguyªn liÖu sÏ biÕn mÊt"},{"Wrapping Leaf",1130,1,0},{"Red Bean",1134,1,5},{"İch Trİ Tèng",1120,1}}
	...
	Tb_zong_zi_info[1124] = {{"b¸nh İt ch©n gµ","Néi <color=yellow>10<color> phót sau khi sö dông, Sinh lùc t¨ng thªm <color=yellow>30##<color>","returnlife_improve()",50,10000,"delete_all","toµn bé nguyªn liÖu sÏ biÕn mÊt"},{"Wrapping Leaf",1130,1,0},{"Roasted Drumstick",1137,1,8},{"Mugwort Zongzi",1123,1}}
	Tb_zong_zi_info[1125] = {{"b¸nh İt B¸c B¶o","Néi <color=yellow>3<color> phót sau khi sö dông, toµn bé n¨ng lùc t¨ng thªm <color=yellow>60<color> ®iÓm","returnability_improve()",33,40000,"delete_all","toµn bé nguyªn liÖu sÏ biÕn mÊt"},{"Wrapping Leaf",1130,1,0},{"Millet Seed",1138,1,9},{"b¸nh İt ch©n gµ",1124,1}}
	Tb_zong_zi_info[1126] = {{"joyful cake","Cã thÓ ®æi 1 quyÓn <color=yellow>S­ M«n lÖnh th­<color> ë ®©y hoÆc nhËn <color=yellow>100-200<color> ®iÓm danh väng ngÉu nhiªn","returnbook_renation_add()",33,80000,"1125","Tæn thÊt 1 c¸i B¸nh İt b¸t b¶o"},{"b¸nh İt B¸c B¶o",1125,3}}
	Tb_zong_zi_info[1127] = {{"lucky cake","Cã thÓ ®Õn gÆp ta ®Ó ®æi 1 quyÓn <color=yellow>MËt TŞch S­ m«n<color>","returniron_stone_add()",33,100000,"1126"," sÏ tæn thÊt 1 B¸nh İt Vui vÎ"},{"joyful cake",1126,3}}
	Tb_zong_zi_info[1128] = {{"quartz cake","Cã thÓ ®æi 1 quyÓn mËt tŞch <color=red>L¨ng Ba Vi Bé<color>","returnsecret_book_add()",20,400000,"1127"," sÏ tæn thÊt 1 B¸nh İt May m¾n"},{"lucky cake",1127,3}}
...
function zongzi_get(zongzi_num)
	...
	if add_flag == 1 then
		...
		WriteLog("[TÕt §oan Ngä 08] ng­êi ch¬i	"..GetName().."	Reward"..zongzi_num.."c¸i"..Tb_zongzi_08[zongzi_diff][1].." thÊt b¹i, kı hiÖu:"..add_flag)
	end
	--ÆäËü½±ÀøÔö¼Ó
	--Íâ×°ºÍÍâ×°Ôö¼Ó
	if dragon_boat_08_time_chk() == 1 then
		...
		for i = 1,3 do
			...
			if strfind(tostring(suit_got),tostring(add_seq))  == nil then
				...
				if add_flag == 1 then
					...
					WriteLog("[TÕt §oan Ngä 08] ng­êi ch¬i	"..GetName().."	Reward"..Tb_suit_prize[add_seq][player_body][1].." thÊt b¹i, kı hiÖu:"..add_flag)
				end
				--¼ÇÂ¼Êı¾İ
		...
		for i = 1,getn(Tb_baiyu_prize) do
			if ran_num <= Tb_baiyu_prize[i][1] then
				...
				if add_flag == 1 then
					...
					WriteLog("[TÕt §oan Ngä 08] ng­êi ch¬i	"..GetName().."	Reward"..Tb_baiyu_prize[i][3].." thÊt b¹i, kı hiÖu:"..add_flag)
				end
				break
			end
		end
...
function yupin_chg(prize_diff)
	...
	if prize_diff == 1 then
		...
		Msg2Player("Player '"..GetName().." t¹i TruyÒn nh©n n­íc Së' dïng 1 B¸nh İt Ngù PhÈm ®æi 1 "..Tb_yupin_prize[prize_diff][4].."!")
	elseif prize_diff == 2 or prize_diff == 3 then
		...
		Msg2Player("Player '"..GetName().." t¹i TruyÒn nh©n n­íc Së' dïng 1 B¸nh İt Ngù PhÈm ®æi "..Tb_yupin_prize[prize_diff][1].."!")
		add_flag = 1
	end
	if add_flag ~= 1 then
		WriteLog("[TÕt §oan Ngä 08] ng­êi ch¬i	"..GetName().."	Reward"..Tb_yupin_prize[prize_diff][4].." thÊt b¹i, kı hiÖu:"..add_flag)
	end
...
--Ô­ÓĞôÕ×Ó´¦Àí
	...
	function happy_zongzi_prize(prize_diff)
		...
		--¿ªÊ¼¶Ô»°
		local prize_dia = "<color=green>Chu State Survivor<color>: <color=yellow>Joyful Rice Cake<color> exchanges for <color=yellow>"..Tb_happy_zong_zi_prize_name[prize_diff].."<color> tû lÖ vµ trŞ sè nh­ sau!"
		for i = 1,5 do		--Îå¼¶½±Àø
	...
	function prize_get_dtm(prize_diff)
		...
		Talk(1,"","<color=green>Di téc Së quèc<color>: <color=yellow>B¸nh İt Vui vÎ<color> mçi ngµy chØ cã thÓ ®æi <color=yellow>"..happy_zong_zi_restrict_num.."<color> lÇn, ®©y lµ lÇn ®æi thø <color=yellow>"..(num_save+1).."<color> for today!")
