...
function main()
	if CanAcceptTask(TASK_LOVESTORY2,2) == 1 then
		Talk(5,"","How annoying!",
				...
				"Ta v× viÖc nµy mµ buån phiÒn, muèn chuÈn bÞ 1 sè lÔ vËt nh­ng l¹i thiÕu <color=red>Hïng Hoµng töu<color>.",
	...
	elseif CanAcceptTask(TASK_LOVESTORY2,4) == 1 then
		Talk(4,"","Cã thÓ mua ®­îc Hïng Hoµng töu kh«ng?",
				...
				"ThËt lµ tèt qu¸, lÔ phÈm c¬ b¶n ®· ®Çy ®ñ, ®Ó ta chuÈn bÞ 1 chót sÏ ®Õn B¹ch phñ.",
	...
	elseif CanAcceptTask(TASK_LOVESTORY2,5) == 1 then
		Say("C¸c vÞ vÉn ch­a gÆp B¸ch c« n­¬ng sao?",0)
	elseif CanAcceptTask(TASK_LOVESTORY2,6) == 1 then
		Talk(6,"","C«ng tö ®· chuÈn bÞ mang sè lÔ phÈm ®Õn B¹ch c« n­¬ng ch­a?",
				...
				"ThÕ nµy ®i, <color=red>D­¬ng Trung ®éng tÇng 2<color> cã mét sè <color=red>b¸nh lÔ<color> mïi vÞ rÊt ngon, thÝch hîp lµm quµ tÆng. Ta sÏ mang vÒ 12 c¸i cho c«ng tö.",
				"Xin ®a t¹ c¸c vÞ.")
		local OldPlayerIndex = PlayerIndex
		for i=1,2 do
			PlayerIndex = GetTeamMember(i)
	...
	elseif CanAcceptTask(TASK_LOVESTORY2,7) == 1 then
		if GetItemCount(2,0,360) >= 12 then
			...
			Talk(4,"","§©y lµ b¸nh lÔ, c«ng tö h·y gãi l¹i mang tÆng B¹ch c« n­¬ng.",
					...
					"§­îc! §Ó ta chuÈn bÞ ngùa råi sÏ ®i liÒn.")
			local OldPlayerIndex = PlayerIndex
			for i=1,2 do
			...
			Say("C¸c vÞ kh«ng ph¶i nãi ë <color=red>D­¬ng Trung ®éng tÇng 2<color> cã mét sè <color=red>b¸nh lÔ<color> mïi vÞ rÊt ngon, thÝch hîp lµm quµ tÆng, muèn gióp ta t×m 12 c¸i ®ã sao?",0)
		end
