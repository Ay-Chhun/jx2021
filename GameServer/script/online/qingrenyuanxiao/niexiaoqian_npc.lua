...
function main()
	...
	elseif CanAcceptTask(TASK_LOVESTORY3,3) == 1 then
		if GetItemCount(2,2,33) >= 3 then
			DelItem(2,2,33,3)
			Talk(6,"","ChÝnh lµ c¸i nµy, cã thÓ hãa gi¶i ®Þnh th©n phï råi. MÆc dï kh«ng bÞ bã buéc, nh­ng còng kh«ng thÓ ch¹y tho¸t.",
					"VËy th× h·y ®Ó ta ®¸nh b¹i L·o L·o",
					"Kh«ng thÓ, Quû Vùc L·o L·o qu¸ nham hiÓm, c¸c vÞ kh«ng ®¸nh næi ®©u",
					"Ph¶i thö míi biÕt.",
					"Th«i ®­îc, muèn tiªu diÖt ®­îc Quû Vùc L·o L·o ph¶i cã <color=red>YÕn XÝch Hµ b¶o kiÕm<color>, hiÖn ®ang ë chç Ninh c«ng tö. LÇn tr­íc YÕn XÝch Hµ tÆng b¶o kiÕm cho <color=yellow>Th¸i ThÇn<color> ®Ó trõ tµ ma.",
					"VËy ®Ó ta ®Õn chç Ninh Th¸i ThÇn.")
			local OldPlayerIndex = PlayerIndex
			for i=1,2 do
				PlayerIndex = GetTeamMember(i)
				SetTask(TASK_LOVESTORY3,4)
				TaskTip("T×m Ninh Th¸i ThÇn lÊy YÕn XÝch Hµ b¶o kiÕm.")
			end
			PlayerIndex = OldPlayerIndex
		else
			Say("§Þnh th©n phï sî nhÊt lµ m¸u chã, h·y t×m <color=red>3 chÐn H¾c KhuyÓn Linh HuyÕt<color> ®Ó hãa gi¶i nã.",0)
		end
	elseif CanAcceptTask(TASK_LOVESTORY3,4) == 1 then
		Say("Th«i ®­îc, muèn tiªu diÖt ®­îc Quû Vùc L·o L·o ph¶i cã <color=red>YÕn XÝch Hµ b¶o kiÕm<color>, hiÖn ®ang ë chç Ninh c«ng tö. LÇn tr­íc YÕn XÝch Hµ tÆng b¶o kiÕm cho <color=yellow>Th¸i ThÇn<color> ®Ó trõ tµ ma.",0)
