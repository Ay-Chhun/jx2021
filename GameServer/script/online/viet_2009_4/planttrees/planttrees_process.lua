...
-----¼ì²é½ñÌìÖÖµÄÊ÷ÊÇ·ñÒÑ¾­µ½ÔÊĞíµÄ×î´óÊıÁ¿--------------------------------------
		if GetTask(TASK_PLANT_TREE_COUNT) >= PLANT_TREE_LIMIT then
			Talk(1, "", "Today you have planted <color=yellow>".. GetTask(TASK_PLANT_TREE_COUNT) .."<color> c©y råi, ®îi ngay mai h·y tiÕp tôc nhĞ.");
			return 0;
		end
...
-----´Ë´¦ÔËÓÃÊ÷³öÏÖµÄ¸ÅÂÊÀ´µÃµ½ÖÖÏÂÊ÷µÄÀàĞÍ-----------------------------------------------------
		...
		WriteLogEx("Hoat dong thang 4","plant a tree",1,TREE_TYPE[nTreeType][2])
