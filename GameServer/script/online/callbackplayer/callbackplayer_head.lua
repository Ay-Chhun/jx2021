...
function tiandi_choujiang()
	...
	if nNum < 4 then
		...
		elseif nRand <= 2 then
			...
			Msg2Global("Player"..GetName().."Tham gia rót th¨m may m¾n Vâ l©m tranh b¸ nhËn ®­îc 1 bé Thiªn §Şa HuyÒn Hoµng!");
			AddGlobalCountNews("Player"..GetName().."Tham gia rót th¨m may m¾n Vâ l©m tranh b¸ nhËn ®­îc 1 bé Thiªn §Şa HuyÒn Hoµng!",2);
...
--	end
	...
	elseif nType == 2 then
		...
		if GetUnitCurStates(nNpcIdx,4) >= 100 then
			...
			Talk(1,"",tShifu[nIdx][5].."Our sect has <color=yellow>"..GetUnitCurStates(nNpcIdx,5).."<color> ®Ö tö dµnh th¾ng lîi khi tû vâ víi ®Ö tö m«n ph¸i kh¸c. Ch­ëng  m«n rÊt h·nh diÖn, quyÕt ®Şnh cho mçi ®Ö tö 2 Tiªu KiÕp t¸n vµ 20 tİn vËt trang bŞ S­ M«n 4, hy väng mäi ng­êi tiÕp tôc cè g¾ng!");
		else
