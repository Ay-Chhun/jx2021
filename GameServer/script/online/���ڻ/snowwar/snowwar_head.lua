...
function SW_CreateSnowWarNpc()
	...
	local npcIndex = CreateNpc("Giang hå hµo kiÖt","Coachman_Snowball Fight",nFieldID,1665,3652)
	SetNpcScript(npcIndex,"\\script\\online\\´º½Ú»î¶¯\\³É¶¼Ñ©ÕÌ´«ËÍÈËout.lua")
	npcIndex = CreateNpc("Giang hå hµo kiÖt","Coachman_Snowball Fight",nFieldID,1665,3224)
	SetNpcScript(npcIndex,"\\script\\online\\´º½Ú»î¶¯\\³É¶¼Ñ©ÕÌ´«ËÍÈËout.lua")
	npcIndex = CreateNpc("Giang hå hµo kiÖt","Coachman_Snowball Fight",nFieldID,1857,3226)
	SetNpcScript(npcIndex,"\\script\\online\\´º½Ú»î¶¯\\³É¶¼Ñ©ÕÌ´«ËÍÈËout.lua")
	npcIndex = CreateNpc("Giang hå hµo kiÖt","Coachman_Snowball Fight",nFieldID,1856,3652)
	SetNpcScript(npcIndex,"\\script\\online\\´º½Ú»î¶¯\\³É¶¼Ñ©ÕÌ´«ËÍÈËout.lua")	
end;
--ÉèÖÃÍæ¼ÒµÄÒ»Ğ©×´Ì¬
...
function SW_JoinSnowWar()
	...
	if NewWorld(nFieldID,tBORN_POS[nRandPos][1],tBORN_POS[nRandPos][2]) == 1 then
		...
		WriteLog("[Snowball Fight]:"..GetName().."Vµo chiÕn ®Şa NewWorld thÊt b¹i!!!");
	end;
end;
...
function SW_ProcessWaitOutTimer()
	...
	if nLoop == 0 then
		...
		if mod(nLoop,2) == 0 then
			Msg2MSAll(MISSION_ID,"TÊt c¶ mäi ng­êi dïng ®iÓm tİch lòy cña m×nh ®æi phÇn th­ëng."..(nLoop/2).." minutes.");
		end;
	end;
end;
--Ñ©ÕÌ¼ò½é
