...
function snowwar_enter_main()
	...
	Say("<color=green>"..g_sNpcName.."<color>: Esteemed"..strSex.."lÏ nµo kh«ng muèn nh©n c¬ héi ch¬i ®ïa tháa thİch mét lÇn hay sao?",getn(selTab),selTab)	
end
...
function go()
	...
	if GetPlayerRoute() == 0 then
		Talk(1,"main","<color=green>"..g_sNpcName.."<color>: Esteemed"..strSex.."<color=yellow>gia nhËp m«n ph¸i<color> míi ®­îc tham gia nĞm tuyÕt.");
		return 0;
	end;
	if GetLevel() <= 10 then
		Talk(1,"main","<color=green>"..g_sNpcName.."<color>: Esteemed"..strSex.."§îi ®Õn cÊp <color=yellow>11<color> råi h·y quay l¹i nhĞ!");
		return 0;
	end;
...
function see_rank()
	Talk(1,"main","<color=green>"..g_sNpcName.."<color>: You have knocked down <color=yellow>"..GetTask(PT_KILL_COUNT).."<color> ng­êi, bŞ ng·<color=yellow>"..GetTask(PT_DEATH_COUNT).."<color> lÇn.");
end;
--=========================================================================================
