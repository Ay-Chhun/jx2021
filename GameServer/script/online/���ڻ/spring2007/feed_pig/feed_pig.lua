...
function OnUse(goods_index)
	if zgc_pub_day_turn(1) > 13576 then			--»î¶¯Ê±¼äÅÐ¶Ï
		return
	end
...
function pig_baby_use(goods_id)					--Öí±¦±¦µÄÊ¹ÓÃ
	...
	elseif goods_id == Goods_Id_golden_pig then
		Say("Sö dông vËt phÈm nµy cã thÓ gäi ®­îc Hoµng kim tr­, ng­¬i ®ång ý chø?",
		3,
...
function pig_create(pig_color,pig_sex,goods_id)
	...
	SetTask(TaskID_spring2007_pig_sex,pig_sex)			--ÖíµÄÐÔ±ð
	SetTaskTemp(165,goods_id)
	--ÏÈÉ¾³ýÄ¿Ç°µÄ¸ú³è£¬È»ºóÌí¼ÓÐÂµÄ¸ú³è£¬²¢ÔÚ976ÖÐ¼ÇÂ¼ÏÂ¸ú³èÐÅÏ¢
...
function pig_create_rtn(pig_name)
	...
	if DelItem(2,0,goods_id,1) ~= 1 then					--ÅÐ¶ÏÉ¾³ýµÀ¾ßÊÇ·ñ³É¹¦
