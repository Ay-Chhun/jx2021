...
function get_translife_item()
	if GetTranslifeCount() <= 0 then -- or GetTaskTrans() <= 0 or GetTranslifeFaction() == 0 then
		Talk(1,"",szNpcName.."Hçn Nguyªn C«ng tung hoµnh thiªn h¹, mÊy ai lÜnh héi ®­îc?")
		return	
	end	
...
function confirm_get_translife_item()
	...
	local nTypeNew = GetPlayerRebornParam(1);
	---------chuyÓn sinh 7
	...
	--	local nBody_cs6 = GetBody()
		...
		for i = 1, 3 do
			gf_PSAddNewItem(tb_translife_cloth_cs7_nhanlai[nType_cs7][nBody][i], "NhËn l¹i trang bÞ")
		end
		for i=1, getn(tb_translife_cloth[nType][nTransCount]) do
			local G,D,P,nCount, _, lvatt1, att1, lvatt2, att2, lvatt3, att3  = gf_UnPack(tb_translife_cloth[nType][nTransCount][i][2])
			P = P + GetBody() - 1
	...
	if GetPlayerRebornParam(0) == 3 then
		...
		local nBody = GetBody()	
		gf_AddItemEx(tb_translife_seal_cs6[nType_cs8][2], tb_translife_seal_cs6[nType_cs8][1])		
		if GetLevel() < 79 then
			Talk(1,"","§¹i hiÖp ch­a ®ñ yªu cÇu ®Ó nhËn l¹i trang bÞ")	
			return
		end
	...
	--	local nBody_cs6 = GetBody()
		...
		for i = 1, 3 do
			gf_AddItemEx(tb_translife_cloth_cs8_nhanlai[nType_cs8][nBody][i], "NhËn l¹i trang bÞ")
		end
		for i=1, getn(tb_translife_cloth[nType][nTransCount]) do
			local G,D,P,nCount, _, lvatt1, att1, lvatt2, att2, lvatt3, att3  = gf_UnPack(tb_translife_cloth[nType][nTransCount][i][2])
			P = P + GetBody() - 1
	...
	if GetPlayerRebornParam(0) == 4 then
		...
		local nBody = GetBody()	
		gf_AddItemEx(tb_translife_seal_cs6[nType_cs9][2], tb_translife_seal_cs6[nType_cs9][1])		
		if GetLevel() < 79 then
			Talk(1,"","§¹i hiÖp ch­a ®ñ yªu cÇu ®Ó nhËn l¹i trang bÞ")	
			return
		end
	...
	--	local nType_cs6 = GetPlayerRebornParam(1) -- xãa trang bi CS6
	--	local nBody_cs6 = GetBody()
		for i=1, getn(tb_translife_cloth_cs9_nhanlai[nType_cs9][nBody]) do
			local G,D,P,nCount, _, lvatt1, att1, lvatt2, att2, lvatt3, att3  = gf_UnPack(tb_translife_cloth_cs9_nhanlai[nType_cs9][nBody][i])
			--P = P + GetBody() - 1
			lvatt3 = lvatt3 or 0
			att3 = att3 or 0
			BigDelItem(G,D,P,BigGetItemCount(G,D,P))
		...
		for i = 1, 3 do
			gf_AddItemEx(tb_translife_cloth_cs9_nhanlai[nType_cs9][nBody][i], "NhËn l¹i trang bÞ")
		end
		for i=1, getn(tb_translife_cloth[nType][nTransCount]) do
			local G,D,P,nCount, _, lvatt1, att1, lvatt2, att2, lvatt3, att3  = gf_UnPack(tb_translife_cloth[nType][nTransCount][i][2])
			P = P + GetBody() - 1
