...
function main()
	local CR_Menu1 = {
		"Vµo Khu nÐm tuyÕt (Tiªu hao 10 vµng tÆng 10 TiÓu TuyÕt CÇu)/CR_DoEnter",
		"Buy snowball ammo/CR_DoShowSaleList",
		"Gi¶i thÝch ho¹t ®éng/CR_Information",
		"Leave/gf_DoNothing"
	}
	local CR_Menu2 = {
...
function CR_DoEnter()
  ...
  mapID = SB_MapMap[mapID];
	...
	if 1 ~= SB_CheckEquipment() then
		Talk(1, "", "Xin lçi, h·y th¸o vò khÝ xuèng");
		return
	end
...
function CR_DoBuy(index)
	...
	if 1 == gf_Judge_Room_Weight(1, itemCount, "") then
		if 1 == Pay(cost) then
			...
			gf_AddItemEx2(tmpItemID, itemName, "§¸nh trËn tuyÕt", "Buy snowball ammo");
		end
	end
	CR_DoShowSaleList();	
end
...
function CR_Information()
	Talk(1,"","Sau khi ®¹i hiÖp vµo khu nÐm tuyÕt, ®èi tho¹i víi Thiªn S¬n TuyÕt Yªu ë chÝnh gi÷a, cã thÓ triÖu håi TuyÕt Qu¸i. Sau khi ®¸nh b¹i tÊt c¶ TuyÕt Qu¸i sÏ xuÊt hiÖn l­ît tiÕp theo, tæng céng cã 10 l­ît. Sau khi ®¸nh xong 10 l­ît th× ®èi tho¹i víi Thiªn S¬n TuyÕt Yªu, ®¸nh b¹i BOSS sÏ nhËn ®­îc phÇn th­ëng phong phó. Trong khu nÐm tuyÕt, chØ cã thÓ dïng kü n¨ng tuyÕt cÇu, kh«ng thÓ trang bÞ vò khÝ.")
end
