...
tGaojiMiji = { --Ô­À´µÚ2ÁÐÎª¸ß¼¶ÐÂ·¨ ºóÀ´È¡ÏûÁË
	[2] = {{"Kim Cang Bat Nha Kinh",0,107,166},{"Kim Cang Bat Nha Kinh",0,107,166},{"Vajra Demon-Subduing Sutra",0,107,1},{"Kim Cang Phôc Ma T©m Ph¸p",0,107,2}},
	[3] = {{"Vo Tran Bo De Kinh",0,107,168},{"Vo Tran Bo De Kinh",0,107,168},{"V« TrÇn MËt tÞch",0,107,5},{"V« TrÇn T©m Ph¸p",0,107,6}},
	...
	[9] = {{"Azure Sea Silent Sound Score",0,107,171},{"Azure Sea Silent Sound Score",0,107,171},{"Azure Sea Score",0,107,11},{"BÝch H¶i T©m Ph¸p",0,107,12}},
	...
	[17] = {{"Tran Quan Phi Long Thuong Manual"	,0,107,176},{"Tran Quan Phi Long Thuong Manual"	,0,107,176},{"TrÊn Qu©n MËt tÞch"	,0,107,21},{"TrÊn Qu©n T©m Ph¸p",0,107,22}},
	...
	[20] = {{"U Minh Phong Ma Manual",0,107,178},{"U Minh Phong Ma Manual",0,107,178},{"Nether Ghost Record",0,107,25},{"U Minh T©m Ph¸p",0,107,26}},
	[21] = {{"Linh Co Huyen Ta Manual",0,107,179},{"Linh Co Huyen Ta Manual",0,107,179},{"Linh Cæ MËt tÞch",0,107,27},{"Linh Cæ T©m Ph¸p",0,107,28}}
};
...
function confirm_change_miji(nType)
	if nType == 1 then
		...
		if add_flag == 1 then
			...
			Msg2Global("Player"..GetName().."Dïng Vâ L©m B¶o §iÓn h·y söa ch÷a phôc håi Ngò Hµnh MËt TÞch thµnh Ngò Hµnh MËt TÞch toµn tËp!");
	  ...
	  if GetItemCount(0,112,78) < 1 then
 	  ...
 	  if DelItem(0,112,78,1) == 1 and DelItem(2,1,3206,1) == 1 then
 	 		...
 	 		if add_flag == 1 then
 	 			...
 	 			Msg2Global("Player"..GetName().."Dïng Vâ L©m B¶o §iÓn h·y söa ch÷a phôc håi L¨ng Ba Vi Bé thµnh L¨ng Ba Vi Bé toµn tËp!");
...
function OnPutinCheck(nParam,nItemIdx,nID1,nID2,nID3)
	...
	Say("<color=green>Vâ L©m B¶o §iÓn<color>: §©y kh«ng ph¶i lµ vËt ta cÇn, thø ta cÇn lµ <color=yellow>"..tGaojiMiji[nRoute][3][1].." or"..tGaojiMiji[nRoute][4][1].."<color>, ng­¬i h·y xem kü l¹i.",0);
	return 0
end
...
function OnPutinComplete(nParam)
	...
	if DelItemByIndex(tbDelItemInfo[1][1],-1) == 1 and DelItem(2,1,3206,1) == 1 then
		...
		if nAddFlag == 1 then
			...
			WriteLog("Vâ L©m B¶o §iÓn: Ng­êi ch¬i"..GetName().." has exchanged the manual"..tGaojiMiji[nRoute][1][1]);
		else
			WriteLog("Vâ L©m B¶o §iÓn: Ng­êi ch¬i"..GetName().."Exchange the manual"..tGaojiMiji[nRoute][1][1].." thÊt b¹i, ký hiÖu:"..nAddFlag);
		end
	else
		Talk(1,"","<color=green>Vâ L©m B¶o §iÓn<color>: VËt ng­¬i giao h×nh nh­ cã chót vÊn ®Ò, phiÒn ng­¬i xem l¹i gióp.");
	end
