...
function open_panlongbi()
	...
	if (DelItem(2, 1, 1001, 1) == 1) then
		...
		if nLimitTime == 0 then
			...
			Say("B¹n nhËn ®­îc <color=yellow>2000 tiÒn game<color>! H«m nay b¹n ®· dïng Hßa ThÞ BÝch lÇn thø <color=yellow>"..(nCurUseTime+1).."<color>. Dùa vµo ®¼ng cÊp hiÖn t¹i, h«m nay b¹n cã thÓ dïng <color=yellow>"..nLimitTime.."<color> He Shi Bi.", 0)
