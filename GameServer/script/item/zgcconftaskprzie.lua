...
function OnUse(id)
	...
	if GetItemParam(id,1) == 0 then											--Ê×ÏÈÅĞ¶ÏÔÉÌúÊÇ·ñ»¹ÔÚ
		if zgc_conf_task_iron_add(prize_level) == 1 then
			if prize_level == 7 and random(1000) <= prize_zhuangbei_jilv[7] then
				...
				if add_flag == 1 then
					AddGlobalNews("Player"..GetName().."Dïng 'B¸ch B¶o r­¬ng Bé Phi Yªn' më 50 quyÓn 'Th¸i DŞch Hçn Nguyªn C«ng §å'!")
	...
	elseif prize_level == 7 then
		...
		AddGlobalNews("Player"..GetName().."Sö dông \'B¸ch B¸u R­¬ng cña Bé Phi Yªn\' më Hoµng Kim Vò Khİ:"..prize_att[7][prize_seq][5])
	else
...
function player_fini_cond()
	...
	string_return = "("..turn_num_sum.." round,"..goods_num_count.." ®iÓm tİch lòy)"
