   			...
   			if SubWorld ~= -1 then
   				...
   				elseif nMapState == 1 then	
   					...
   					if GetMSPlayerCount(SINGLE_TEAM_MISSION_ID,1) > 0 and GetMSPlayerCount(SINGLE_TEAM_MISSION_ID,2) > 0 then
   						...
   						szSay = szSay..","..format("%q", SingleTeamMapTab[i][3].."(Tr¹ng th¸i: §îi khai chiÕn, hai phe ®èi chiÕn lµ ®éi "..nCaptainName_1.." and team"..nCaptainName_2.." )/#OpenSingleTeamRoom("..i..")");
   					...
   					elseif GetMSPlayerCount(SINGLE_TEAM_MISSION_ID,1) > 0 or GetMSPlayerCount(SINGLE_TEAM_MISSION_ID,2) > 0 then
   						...
   						PlayerIndex = OldPlayer;
   						szSay = szSay..","..format("%q", SingleTeamMapTab[i][3].."(Tr¹ng th¸i: §îi thi ®Êu, tin tøc ®éi ngò:"..nCaptainName_1..", has"..nTeamSize_1.." ng­êi.)/#OpenSingleTeamRoom("..i..")");
   					end
   				--±ÈÈüÒÑ¾­¿ªÊ¼
   				elseif nMapState == 2 then
   					...
   					PlayerIndex = OldPlayer;
   					szSay = szSay..","..format("%q", SingleTeamMapTab[i][3].."(Tr¹ng th¸i: §ang thi ®Êu, hai phe ®èi chiÕn lµ ®éi "..nCaptainName_1.." and team"..nCaptainName_2.." )/#OpenSingleTeamRoom("..i..")");
   				elseif nMapState == 3 then
   					szSay = szSay..","..format("%q", SingleTeamMapTab[i][3].."(Status: Contest ended)/#GameOverTalk");
   				end
   			end   			
   			SubWorld = OldWorld;	   					
   		end 			
...
function OnChanceRoom(index)
    if (GetTaskTemp(JOINROOMINDEX)>0 and GetTaskTemp(JOINROOMINDEX)~=index) then
        Say("You have reserved a battle ground"..GetTaskTemp(JOINROOMINDEX)..", h·y chän vµo trËn ®Şa ®· ®Æt!",0);
    else
	    ...
	    if (ms_state <= 0) then
		    Say("N¬i ®©y lµ ®Êu tr­êng ®¬n ®Êu l«i ®µi cho c¸c Nh©n sÜ giang hå tham gia ®¬n ®Êu tû thİ. HiÖn ch­a cã ®Êu thñ nµo ®¨ng kı thi ®Êu, c¸c h¹ cã høng thó kh«ng? <color=yellow>TrËn ®¬n ®Êu l«i ®µi hiÖn më miÔn phİ, h·y chíp lÊy thêi c¬!", 3, "Accept/OnRegister", "What are the duel rules?/OnHelp", "Let me prepare a bit more/OnCancel");
...
function OnHelp()
    SetTaskTemp(JOINROOMINDEX,0);
	Talk(7, "",	"§¬n ®Êu l«i ®µi lµ n¬i c¸c ®¹i hiÖp cao thñ trªn giang hå tû thİ vâ nghÖ, ph©n tµi cao thÊp. L«i ®µi lµ trËn ®Şa thi ®Êu c«ng b»ng cho hai phe tham dù!",	"Muèn tiÕn hµnh trËn tû vâ l«i ®µi cÇn ®Õn ®¨ng kı thi ®Êu víi ta!",	"After both sides form teams, the team leaders come to register to join the match!",	"Do trËn ®Şa h¹n hÑp, khi mét l«i ®µi nµo ®ã ®ang tiÕn hµnh thi ®Êu th× l«i ®µi kh«ng tiÕp nhËn b¸o danh!",	"B¸o danh thµnh c«ng, hai phe b¾t ®Çu chuÈn bŞ thi ®Êu. Thêi gian vµo trËn cña hai phe lµ 3 phót. Trong vßng 3 phót, c¸c ®Êu thñ ph¶i khÈn tr­¬ng vµo ®Êu tr­êng tiÕn hµnh thi ®Êu!" , 	"Thêi gian thi ®Êu lµ 10phót. Trong 10 phót, hai phe ch­a ph©n th¾ng b¹i sÏ ®­îc xö hßa!",	"Trong lóc thi ®Êu, bÊt kú bªn nµo bŞ rít m¹ng hoÆc vÒ thµnh sÏ bŞ xö b¹i!");
end;
...
--		end;
	...
	for i = 1, 2 do 
		PlayerIndex = gf_GetTeamMember(i);
--		Pay(V);
        TaskTip("§éi ngò cña ng­¬i ®· b¸o danh tham gia trËn ®Şa"..MapTab[GetTaskTemp(JOINROOMINDEX)][3].."PK match!");
...
local str_news = "";
	...
	if nMapState == 0 then
  	...
  	SetMissionV(MULTIMAP_NPC_INDEX,nNpcIndex)
		...
		str_news = GetPlayerInfo(PlayerIndex)..GetName().."is at"..MultiMapTab[nMapIndex][9].."L«i ®µi s¾p xÕp QuÇn hïng chiÕn. Mêi c¸c anh hïng tham gia thİ luyÖn!",
		Msg2Global(str_news);
...
local nCaptainName = "";
	...
	if SubWorld ~= -1 then
		...
		elseif nMapState == 2 or (GetMSPlayerCount(SINGLE_TEAM_MISSION_ID,1) > 0 and GetMSPlayerCount(SINGLE_TEAM_MISSION_ID,2) > 0) then
			Say("TrËn ®Şa nµy ®· b¾t ®Çu thi ®Êu, ®éi ngò hai phe do <color=yellow>"..GetMissionS(CAPTAIN_NAME).."<color> and <color=yellow>"..GetMissionS(CAPTAIN_NAME+1).."<color> dÉn d¾t. Cã muèn vµo trËn ®Şa nµy xem thi ®Êu?",
			2,
		...
		if PlayerIndex > 0 then
			for i=3,10 do
				if GetMissionS(i) == "" then
					...
					Say("Do <color=yellow>"..nChallengeName.."<color> leading (consisting of: <color=yellow>"..nChallengeTeamSize.."<color> ng­êi) mêi ng­¬i tû vâ. Ng­¬i ®ång ı kh«ng?",
					2,
