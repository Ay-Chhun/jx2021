...
szNpcName =  "<color=green>Hoµng Hæ<color>: "
...
function TopBangHoi_DangKy()
	...
	if nLevel <  90 then
		Talk(1,"",szNpcName.."§¼ng cÊp cña c¸c h¹ kh«ng ®ñ 90, kh«ng thÓ ®¨ng kı tham gia!")
		return 0
	end
	if nMaxSkill ~= 1 then
...
function TopBangHoi_CongHien_Done(nOption)
	...
	gf_WriteLogEx(LIB_Award.szLogTitle, LIB_Award.szLogaction, 1, "Contribute"..tbCongHienBang_NguyenLieu[nOption][1].." ®iÓm")
...
function TopBangHoi_NhanThuong_ThanhVien(nRank)
	...
	local tbAward1 = {
		item = {
			...
			{gdp={0,105,164,1,4,-1,-1,-1,-1,-1,-1,-1}}, -- H¾c Hæ
		},
		nExp = 200000000,
	}
	local tbAward2 = {
		item = {
			...
			{gdp={0,105,30011,1,4,-1,-1,-1,-1,-1,-1,-1}}, -- B¸o §en
		},
		nExp = 100000000,
	}
	local tbAward3 = {
...
function TopBangHoi_NhanThuong_BangChu(nRank)
	...
	local nNow = GetTime()
	if (nNow - nLastAssignTime) < (1*24*3600)   then
		Talk(1,"",szNpcName.."Thêi gian t¹i vŞ cña c¸c h¹ İt h¬n 1 ngµy, kh«ng thÓ nhËn th­ëng!")
		return 0
	end
	if nRank == 1 then
		TraoHoaPhungTuongQuan(0)
		TraoMatTichCaoCap20(7)
		local tbAward = {item={{gdp={0,105,160,1,4,-1,-1,-1,-1,-1,-1,-1}}}}
		LIB_Award:Award(tbAward)
	elseif nRank == 2 then
		TraoLoiHoTuongQuan(13)
		Receive_KimXa()		
		TraoMatTichCaoCap20(5)
		local tbAward = {item={{gdp={0,105,30010,1,4,-1,-1,-1,-1,-1,-1,-1}}}}
		LIB_Award:Award(tbAward)
	elseif nRank == 3 then
		TraoMatTichCaoCap20(3)
		
		--Trao Trang bi chiÕn cuång		
		--TraoTrangBiChienCuong()
		local tbSay = {}		
		tinsert(tbSay,"Ta muèn nhËn trang bŞ Kim Xµ V©n Du/#receive_KimXa_top1_faction(1)")
		tinsert(tbSay,"Ta muèn nhËn trang bŞ Kim Xµ ThiÕt HuyÕt/#receive_KimXa_top1_faction(2)")
		tinsert(tbSay,"Ta muèn nhËn trang bŞ Kim Xµ Du HiÖp/#receive_KimXa_top1_faction(3)")
		Say(strNpcName.."C¸c h¹ h·y lùa chän 1 trong 3 bé trang bŞ Kim Xµ: ",getn(tbSay),tbSay)
		--Chän lùa trang søc diÖu d­¬ng
...
function Receive_KimXa()
	local tbSay = {}		
		tinsert(tbSay,"Ta muèn nhËn trang bŞ Kim Xµ V©n Du/#Receive_KimXa_01(1)")
		tinsert(tbSay,"Ta muèn nhËn trang bŞ Kim Xµ ThiÕt HuyÕt/#Receive_KimXa_01(2)")
		tinsert(tbSay,"Ta muèn nhËn trang bŞ Kim Xµ Du HiÖp/#Receive_KimXa_01(3)")
		Say(strNpcName.."C¸c h¹ h·y lùa chän 1 trong 3 bé trang bŞ Kim Xµ: ",getn(tbSay),tbSay)
end

function Receive_KimXa_01(nChoice)	
	
	if nChoice  == 1 then
		LIB_Award:Award(tbKimXaVanDu10)
		Msg2Player("NhËn thµnh c«ng bé trang bŞ Kim Xµ V©n Du")
	elseif nChoice == 2 then
		LIB_Award:Award(tbKimXaThietHuyet10)
		Msg2Player("NhËn thµnh c«ng bé trang bŞ Kim Xµ ThiÕt HuyÕt")
	elseif nChoice == 3 then
		LIB_Award:Award(tbKimXaDuHiep10)
		Msg2Player("NhËn thµnh c«ng bé trang bŞ Kim Xµ Du HiÖp")
	end	
end
