function main()
	...
	elseif	(GetSex()==2 and GetPlayerFaction() ~= 5) then
		Say("C´ n≠¨ng t◊m ta c„ vi÷c g◊?",2,"Phi“n t˚ t˚ Æ≠a ta rÍi ß≠Íng M´n/outtangmen","Nothing/no")
	elseif	(GetSex()~=2 and GetPlayerFaction() == 5) then
		Say("S≠ Æ÷ lπi muËn trËn ra ngoµi ch¨i µ?",2,"ß÷ phÙng m÷nh hπ s¨n/outtangmen","Nothing/no")
	else
		Say("C´ng tˆ! Ta c„ th” giÛp g◊?",2,"Phi“n c´ n≠¨ng Æ≠a ta mÈt Æoπn/outtangmen","Nothing/no")
	end
end;
