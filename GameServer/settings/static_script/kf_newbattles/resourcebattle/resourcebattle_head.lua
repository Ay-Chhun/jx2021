...
function Get_Random_Resource(nMultiple)
	...
	elseif nRandomNum <= 90 then
		Msg2MSGroup(MISSION_ID,"Player"..GetName().."t¹i Nh¹n M«n Quan-Th¶o cèc chiÕn ®µo bíi bõa b·i ¶nh h­ëng ®Õn m«i tr­êng nªn bÞ thæ d©n truy ®uæi, chê t­íng sÜ ®Õn gi¶i cøu.",nCamp);
		local npcIndex = CreateNpc("Hoµn tr­êng Thæ d©n","Forest-guarding Local",GetWorldPos());
...
function Create_Cateran(nCamp,nCount)
	...
	local nNpcIndex = 0;
	for i=1,nCount do
		nNpcIndex = CreateNpc("S¬n tÆc l©u la","Mountain bandit ("..tCampNameZ[nCamp]..")",MAPID,tPos[i][1],tPos[i][2]);
