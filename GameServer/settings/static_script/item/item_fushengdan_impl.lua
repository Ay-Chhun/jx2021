...
fsd_max_xiulan=100
fsd_taiyisuipian = {"Th¸i DÞch Hçn Nguyªn C«ng §å",2, 0, 1082}
fsd_taiyihunyuan = {"Th¸i DÞch Hçn Nguyªn Phæ",2, 0, 1083, 4}
fsd_tNeed = {
	--¾­Ñé Ì«Ò×ËéÆ¬ ÐÞÎª µãÊý
...
function do_xiulian_sure(nItemIdx, nType)
	...
	local nPoint = t[4]
	if 1 == check_and_give_fushengdan(0, nItemIdx) then --ÏÈ¿´¿´ÒÔÇ°ÓÐÃ»ÓÐÐÞÂúµ«ÊÇÃ»Áìµ½½±ÀøµÄÇé¿ö
		return 0
	end
...
function check_and_give_fushengdan(nPoint, nItemIdx)
	if fsd_get_xiulian() >= fsd_max_xiulan then
		...
		local szTitle = format("Chóc mõng tÝch lòy ®­îc %d ®iÓm tu luyÖn, nhËn ®­îc 1 %s", fsd_max_xiulan, t[1])
		Talk(1,"",szTitle)
		return 1
	else
		if nPoint > 0 then
			local szTitle = format("Chóc mõng thµnh c«ng nhËn ®­îc %d ®iÓm tu luyÖn, ®· tÝch lòy %d/%d ®iÓm tu luyÖn, h«m nay cßn ®­îc tu luyÖn %d/%d ®iÓm", 
